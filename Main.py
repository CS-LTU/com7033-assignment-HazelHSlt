...
# Import "healthcare-dataset-stroke-data.CSV" into MongoDB on startup if mongoDB database is empty.
...
# Initialize databases and create tables for both users and admin.
...
def main(): # (Anthropic, 2025)
    ...
    # Run the application.
    try:
        # Enable HTTPS with self-signed certificate, in production SSL certificates are normally purchased or generated by a trusted Certificate Authority, such as DigiCert or GlobalSign. For developement however, self-signed certificates suffice for demonstrating and testing that the implementation works as intended. Unlike HTTP, HTTPS sends data over an encrypted connection, designed to prevent eavesdropping or "packet sniffing" and tampering such as "man in the middle" attacks.
        print("\n" + "="*70)
