...
# This is the Flask-WTF Forms handler, used to sanitise user input and use "Cross-Site Request Forgery" (CSRF) protection. CSRF is when a different malicious website tricks a user's browser, into submitting unwanted actions. These attacks can be carried out using insecure session cookies to make requests without the user's consent. Flask-WTF generates tokens to validate that only session cookies generated from this site can be accepted).
